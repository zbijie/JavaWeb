  ...
  <input type="hidden" id="order_id" name="order_id" value="${order.id }"/>
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="grgz_tab left">
		<tr>
			...
			<td colspan="3"><input name="order_name" id="order_name" type="text" class="input70" value="${order.order_name }" /> 100个字符内</td>
			</tr>
		<tr>
			<th scope="row"><span class="orange3">*</span> 系统编号：</th>
			<td colspan="3"><input name="sys_no" id="sys_no" type="text" class="input50" value="${order.sys_no }" /> 编码格式：CMFJ+年月日（如20140528）+XQ+流水号（数字0001-9999）</td>
			</tr>
		<tr>
			<th scope="row"><span class="orange3">*</span> 需求大小：</th>
			<td colspan="3">
				<p><input name="order_scope" type="radio" value="1" <c:if test='${order.order_scope==1 }'>checked="checked"</c:if> /> 大 （90人天以上）</p>
				<p><input name="order_scope" type="radio" value="2" <c:if test='${order.order_scope==2 }'>checked="checked"</c:if>  /> 中（30-90人天以内，包括90人天）</p>
				<p><input name="order_scope" type="radio" value="3" <c:if test='${order.order_scope==3 }'>checked="checked"</c:if>  /> 小（30人天以下，包括30人天）</p>
			</td>
			</tr>
		<tr>
			<th scope="row"><span class="orange3">*</span> OA工单号：</th>
			<td><input name="oa_no" id="oa_no" type="text" class="input90" value="${order.oa_no }"/></td>
			<th scope="row"><span class="orange3">*</span> 归属系统：</th>
			<td><select class="input90" id="app_sys_id" name="app_sys_id" >
		...
		<tr>
			...
			<td><input name="order_dept" id="order_dept" type="text" class="input90" value="${order.order_dept }"/></td>
			<th scope="row"><span class="orange3">*</span> 需求提出人：</th>
			<td><input name="" type="text" class="input90" value="${order.order_apply_man }" /></td>
		</tr>
		<tr>
			<th scope="row"><span class="orange3">*</span> 开发负责人：</th>
			<td><input name="develop_man" id="develop_man" type="text" class="input90" value="${order.develop_man }"/></td>
			<th scope="row"><span class="orange3">*</span> 执行厂商：</th>
			<td><select name="execute_company" id="execute_company" type="text" class="input90" >
		...
		<tr>
			...
			<td colspan="3">
			<textarea name="" cols="" rows="" style="height:80px; line-height:20px;" class="input90" id="order_detail" name="order_detail">${order.order_detail }</textarea>
			</td>
			</tr>
		<tr>
	...
	<div class="pop_410" id="pop_sure">
		...
		<div class="pop_410_m text_c">
			...
			<div class="mt20 mb10" style="margin-left:120px;">
				...
				<span class="btn_gr2" onclick="hide('cover1','pop_sure');"><a href="${pageContext.request.contextPath}/sys/order_list.action">暂不下发</a></span>
			</div>
		</div>
	</div>
	<!--保存需求 弹出框 结束-->
